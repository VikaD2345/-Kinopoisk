  ...
  useEffect(() => {
    ...
    async function loadCatalog() {
      try {
        ...
        const localById = new Map(initialCatalog.map((item) => [String(item.id), item]))
        const normalized = remoteItems.map((item) => ({
          ...localById.get(String(item.id)),
          ...item,
          id: String(item.id),
          poster: mediaUrl(item.poster ?? item.posterUrl ?? item.poster_url, apiUrl),
          backdrop: mediaUrl(item.backdrop ?? item.backdropUrl ?? item.backdrop_url, apiUrl),
        }))
        ...
        if (error.name !== 'AbortError') {
          console.warn('Backend каталога недоступен, показаны плейсхолдеры:', error)
